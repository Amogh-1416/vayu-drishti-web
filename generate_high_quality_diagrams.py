...
def create_mermaid_files():
    ...
    # Diagram 1: Overall System Architecture (Visually appealing flow)
    arch = """graph TD
    classDef primary fill:#4A90E2,stroke:#000,stroke-width:2px,color:#fff,rx:5px,ry:5px;
    classDef secondary fill:#50E3C2,stroke:#000,stroke-width:2px,color:#000,rx:5px,ry:5px;
    classDef database fill:#F5A623,stroke:#000,stroke-width:2px,color:#fff,rx:5px,ry:5px;
    classDef frontend fill:#E91E63,stroke:#000,stroke-width:2px,color:#fff,rx:5px,ry:5px;
    ...
    # Diagram 2: Machine Learning Data Pipeline (Video Pipeline)
    ml_pipe = """graph LR
    classDef input fill:#9b59b6,stroke:#000,stroke-width:2px,color:#fff;
    classDef process fill:#3498db,stroke:#000,stroke-width:2px,color:#fff;
    classDef model fill:#e67e22,stroke:#000,stroke-width:2px,color:#fff;
    classDef output fill:#2ecc71,stroke:#000,stroke-width:2px,color:#fff;
...
def generate_images():
    # Use mmdc to convert to png with high resolution and clear theme
    files = ['arch.mmd', 'ml_pipe.mmd', 'ws_flow.mmd']
    for file in files:
        base = file.split('.')[0]
        # using 'default' or 'neutral' theme, larger scale (-s 3) for crispness
        cmd = f"mmdc -i paper_assets/{file} -o paper_assets/{base}.png -t neutral -b transparent -s 3"
        print(f"Running: {cmd}")
